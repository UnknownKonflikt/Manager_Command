...
const insertDepartments = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield pool.connect();
        for (const department of seedData) {
            yield pool.query('INSERT INTO department (name) VALUES ($1)', [department.name]);
        }
...
const updateEmployee = () => {
    ...
    pool.query('SELECT * FROM employee', (err, res) => {
        ...
        for (let i = 0; i < res.rows.length; i++) {
            let employee_id = `${res.rows[i].id} ${res.rows[i].first_name} ${res.rows[i].last_name}`;
